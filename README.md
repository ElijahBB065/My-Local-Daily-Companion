...
> **PIVOT NOTICE (read this first):** this project used to be "Local Daily
> Companion," a general daily-life app covering both transit accessibility
> AND air quality/pollen. It has been fully repositioned into a focused
> **Biomedical Health & Respiratory Dashboard** — every subway/train/bus/
> delay/elevator-outage feature has been removed from `app.py` and
> `homepage.py`. Everything below the next section describes the app in
> its **current** (post-pivot) form; sections further down that predate
> the pivot are kept as historical changelog entries and are called out
> where they're no longer accurate.

## What changed in the Biomedical Health pivot

**Removed entirely from `app.py` / `homepage.py`:**
- All transit code, UI, and API calls — the Transit Accessibility &
  Delay Tracker tab, the Transit status tile, transit-flavored language
  in the Daily Briefing, and the wheelchair/elevator-outage alert badge.
- The calls into `briefing.build_daily_briefing()` and
  `outlook.compute_outlook()` — both required a `transit_status`
  argument and unconditionally wove transit language into their output
  with no way to turn it off, which would mean fabricating transit
  content in an app that no longer tracks transit at all. `app.py` now
  builds its own one-sentence respiratory-only briefing and hazard
  score locally instead.
- `transit.py`, `briefing.py`, and `outlook.py` are no longer imported
  by anything and can be deleted from your repo — they're kept in this
  delivery only because the requested edit scope was `app.py` and
  `homepage.py`, not a repo-wide file deletion pass.

**Added:**
- **Personalized Respiratory Health & Bio-Hazard dashboard**, built on
  the existing Open-Meteo Air Quality API integration in `pollen.py`
  (`pollen.get_environmental_reading()` for the combined PM2.5/PM10/
  pollen/AQI hazard category, plus a direct call to
  `pollen.fetch_open_meteo_environmental()` for the four named pollen
  species — ragweed, grass, birch, olive).
- **Dynamic Asthma & Allergen Risk Score** (`app.py`'s
  `compute_respiratory_hazard()`) — collapses the existing 5-tier
  category system (Low / Moderate / High / Very High / Extreme) into
  exactly three tiers: **LOW (Green) / MODERATE (Yellow) / HIGH RISK
  (Red)**, and shifts the effective threshold based on the user's own
  **sensitivity profile** (Not Sensitive / Mild Allergies / Severe
  Asthma), so the same real-world air quality reads as a higher risk
  for someone who says they have severe asthma.
- **Three main-screen cards:** (1) the Overall Respiratory Hazard Score
  — a large color badge with actionable advice ("keep windows closed,
  carry your rescue inhaler," etc.), (2) Dominant Airborne Triggers —
  clean columns for PM2.5, PM10, and the four named pollen species, and
  (3) a compact Sensitivity & Symptom Log summary.
- **Sidebar Sensitivity & Symptom Log widget** — a selectbox for the
  sensitivity profile plus a session-only symptom log (text + severity
  + timestamp, most recent first), so the full interactive control
  lives in the sidebar while the main screen shows a live summary of it.
- The same "mismatched deployment never crashes" defensive pattern used
  throughout this project (see "Keep all files in sync" below) was
  extended to the two brand-new `homepage.py` functions this pivot
  introduces, `render_hazard_score_card()` and
  `render_airborne_triggers_card()`.

---

A sleek, medical-grade Streamlit dashboard for everyday respiratory
health: a Home page, an Air Quality & Respiratory Details tab, a
Community Hub tab, and several personal features layered on top:
...
├── app.py                 # Streamlit entrypoint — sidebar, view switching, the respiratory hazard
│                          #   engine (compute_respiratory_hazard), wires everything together
├── homepage.py            # Home view — logged-out pitch/login/signup, or logged-in "Welcome back"
│                          #   dashboard; also the hazard-score/airborne-triggers card components
...
├── air_quality.py         # Air Quality tab — OpenAQ integration, EPA AQI math, simulated fallback
├── user_profile.py        # Saved locations + notification prefs (asthma/wheelchair toggles and
│                          #   render_personal_alerts() are no longer called from app.py post-pivot)
├── pollen.py              # Live Open-Meteo pollen/PM2.5 + hazard estimate, simulated fallback --
│                          #   the data backbone of the Respiratory Health & Bio-Hazard dashboard
├── exports.py             # .ics calendar event builder (still used); build_shareable_summary() is
│                          #   no longer called from app.py post-pivot (it required transit_status)
├── community.py           # Community Hub tab — town/neighborhood message board, upvotes, digest panel
├── charts.py              # Plotly chart builders (used by air_quality.py)
├── transit.py             # ⚠️ UNUSED since the Biomedical Health pivot — no longer imported by
│                          #   app.py. Safe to delete from your repo; kept here only because this
│                          #   delivery's edit scope was app.py/homepage.py, not a deletion pass.
├── briefing.py            # ⚠️ UNUSED since the pivot — required a transit_status argument and
│                          #   always wove transit language into its output. Safe to delete.
├── outlook.py             # ⚠️ UNUSED since the pivot — same transit_status coupling as briefing.py.
│                          #   Safe to delete.
├── requirements.txt
└── .streamlit/config.toml # Custom theme
```

### How the personal features fit together (post-pivot)

`app.py` computes the selected city's **local time** once per page load
(`cities.now_in_city(city)`), then computes that city's air-quality
reading and environmental/pollen reading **once** using that same local
time (`air_quality.get_current_reading()` and
`pollen.get_environmental_reading()`), then folds them into the Dynamic
Asthma & Allergen Risk Score (`compute_respiratory_hazard()`) and hands
all of that to the Home page, the three main dashboard cards, the local
Daily Respiratory Briefing sentence, and the Air Quality tab. That's a
deliberate design choice, not just tidiness — it guarantees the hazard
banner, the two other cards, and the tab content always agree with each
other and all show the *same* city-local clock, and it means a live
Open-Meteo/OpenAQ request only ever fires once per rerun for each
purpose instead of once per place it's used.
